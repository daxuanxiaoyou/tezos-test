...
class Error_message:
    ...
    def err_amount(self):            return self.make("ERROR AMOUNT")
    ...
    def err_same_minter(self):       return self.make("ERR_SAME_MINTER")
...
## `Operator_all_param` defines type types for the `%update_operators_all` entry-point.
class Operator_all_param:
    def __init__(self, config):
        self.config = config
    def get_type(self):
        t = sp.TRecord(
...
## The class `Ledger_key_ex` defines the key type for the main ledger-ex (big-)map:
##
## - In *“Babylon mode”* we also have to call `sp.pack`.
class Ledger_key_ex:
    def __init__(self, config):
...
## For now a value in the ledger is just the user's nft list. Previous
## versions of the specification required more information; potential
## extensions may require other fields.
class Ledger_value_ex:
    def get_type():
        return sp.TRecord(balance = sp.TSet(t=sp.TNat))
    def make(balance):
        return sp.record(balance = balance)
...
## The link between operators_all and the addresses they operate is kept
## in a *lazy set* of `(owner × operator)` values.
##
## A lazy set is a big-map whose keys are the elements of the set and
## values are all `Unit`.
class Operator_all_set:
    def __init__(self, config):
        self.config = config
    def inner_type(self):
        return sp.TRecord(owner = sp.TAddress,
                          operator = sp.TAddress
                          ).layout(("owner", "operator"))
    def key_type(self):
        if self.config.readable:
            return self.inner_type()
        else:
...
class FA2_core(sp.Contract):
    def __init__(self, config, metadata, **extra_storage):
        ...
        self.operator_all_set = Operator_all_set(self.config)
        self.operator_all_param = Operator_all_param(self.config)
        self.token_id_set = Token_id_set(self.config)
        self.ledger_key = Ledger_key(self.config)
        self.ledger_key_ex = Ledger_key_ex(self.config)
        ...
        self.init(
            ...
            operators_all = self.operator_all_set.make(),
            all_tokens = self.token_id_set.empty(),
            metadata = metadata,
            ledger_ex = self.config.my_map(tkey = sp.TAddress, tvalue = Ledger_value_ex.get_type()),
            **extra_storage
        )
    ...
    def mint_inner(self, to, tokenid, metadata):
        user = self.ledger_key.make(to, tokenid)
        user_ex = self.ledger_key_ex.make(to)
        ...
        sp.if self.data.ledger_ex.contains(user_ex):
            self.data.ledger_ex[user_ex].balance.add(tokenid)
        sp.else:
            tokenSet = sp.set( [tokenid] )
            self.data.ledger_ex[user_ex] = Ledger_value_ex.make(tokenSet)
        
        sp.if ~ self.token_id_set.contains(self.data.all_tokens, tokenid):
            self.token_id_set.add(self.data.all_tokens, tokenid)
    ...
    def mint(self, params):
        ...
        self.mint_inner(params.address, params.token_id, params.metadata)
           ...
           sp.for tx in transfer.txs:
                ...
                if self.config.support_operator:
                    ...
                    sender_verify |= (self.operator_set.is_member(self.data.operators,
                                                                  ...
                                                                  sp.sender,
                                                                  tx.token_id))
                    sender_verify |= (self.operator_all_set.is_member(self.data.operators_all,
                                                                  current_from,
                                                                  sp.sender))
                sp.verify(sender_verify, message = message)
                sp.verify(tx.amount == 1,message = self.error_message.err_amount())
                sp.verify(
                ...
                from_user_ex = self.ledger_key_ex.make(current_from)
                
                sp.verify(
                    (self.data.ledger[from_user].balance >= tx.amount),
                    message = self.error_message.insufficient_balance())
                to_user = self.ledger_key.make(tx.to_, tx.token_id)
                to_user_ex = self.ledger_key_ex.make(tx.to_)
                self.data.ledger[from_user].balance = sp.as_nat(
                    self.data.ledger[from_user].balance - tx.amount)
                #remove token id from set
                self.data.ledger_ex[from_user_ex].balance.remove(tx.token_id)
                sp.if self.data.ledger.contains(to_user):
                    self.data.ledger[to_user].balance += tx.amount
                sp.else:
                    self.data.ledger[to_user] = Ledger_value.make(tx.amount)
                #add tokenid to to_user
                sp.if self.data.ledger_ex.contains(to_user_ex):
                    self.data.ledger_ex[to_user_ex].balance.add(tx.token_id)
                sp.else:
                    tokenSet = sp.set( [tx.token_id] )
                    self.data.ledger_ex[to_user_ex] = Ledger_value_ex.make(tokenSet)
    ...
    def update_token_metadata(self, params):
        ...
        sp.for update in params:
            sp.verify(
                self.token_id_set.contains(self.data.all_tokens, update.token_id),\
                message = self.error_message.token_undefined()
            )
            self.data.token_metadata[update.token_id] = sp.record(
    ...
    def get_token_metadata(self, para):
        sp.set_type(
            para, sp.TRecord(
                token_id = sp.TNat
            ).layout(("token_id")))
        sp.verify(self.data.token_metadata.contains(para.token_id), message = self.error_message.token_undefined())
        sp.result(self.data.token_metadata[para.token_id])
    ...
    @sp.offchain_view(pure = True)
    def get_balance_by_account(self, req):
        sp.set_type(
            req, sp.TRecord(
                owner = sp.TAddress
            ).layout(("owner")))
        user = self.ledger_key_ex.make(req.owner)
        sp.result(self.data.ledger_ex[user].balance)
    ...
    @sp.entry_point
    def update_operators_all(self, params):
        sp.set_type(params, sp.TList(
            sp.TVariant(
                add_operator = self.operator_all_param.get_type(),
                remove_operator = self.operator_all_param.get_type()
            )
        ))
        if self.config.support_operator:
            sp.for update in params:
                with update.match_cases() as arg:
                    with arg.match("add_operator") as upd:
                        ...
                        self.operator_all_set.add(self.data.operators_all,
                                              upd.owner,
                                              upd.operator)
                    with arg.match("remove_operator") as upd:
                        sp.verify(
                        ...
                        self.operator_all_set.remove(self.data.operators_all,
                                                 upd.owner,
                                                 upd.operator)
        else:
            sp.failwith(self.error_message.operators_unsupported())
...
class FA2_administrator(FA2_core):
    ...
    def set_minter(self, params):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.verify(~(self.data.minter == params), message = self.error_message.err_same_minter())
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_administrator, FA2_pause, FA2_core):
    ...
    @sp.offchain_view(pure = True)
    def is_all_operator(self, query):
        sp.set_type(query,
                    sp.TRecord(owner = sp.TAddress,
                               operator = sp.TAddress).layout(
                                   ("owner", "operator")))
        sp.result(
            self.operator_all_set.is_member(self.data.operators_all,
                                        query.owner,
                                        query.operator)
        )
    ...
    @sp.offchain_view(pure = True)
    def admin(self):
        "retuen the admin address."
        sp.result(self.data.administrator)
    
    @sp.offchain_view(pure = True)
    def minter(self):
        "retuen the minter address."
        sp.result(self.data.minter)

    def __init__(self, config, metadata, admin, minter):
        ...
        list_of_views = [
            ...
            , self.admin
            , self.minter
            , self.get_balance_by_account
            , self.is_all_operator
        ]
        ...
        FA2_core.__init__(self, config, metadata, paused = False, administrator = admin, minter = minter)
...
def environment_config():
    return FA2_config(
        ...
        assume_consecutive_token_ids =
            global_parameter("assume_consecutive_token_ids", False),
...
sp.add_compilation_target("FA2_comp", FA2(config = environment_config(),
                            metadata = sp.utils.metadata_of_url("ipfs://QmWnFgV46gz9nhCb8Rnw6xUj6FtXN39bY5Vt2uJnDAs2X4/metadata.json"),
                            admin = sp.address("tz1P9k2ZzJyicfpBitM9D4FjjyaUwFH9oT9i"),
